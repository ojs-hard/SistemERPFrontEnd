...
const SidebarItems = [
    {
        ...
        route: '/personal',
        children: [
            { id: 'staff_card_list', route: '/personal/list', icon:'fa-copy', text: 'Personel Kart Listesi' },
            { id: 'add_staff_card', route: '/personal/add-staff-card', icon:'fa-copy', text: 'Personel Kartı Ekle' },
            { id: 'add_payroll', route: 'add_payroll', icon:'fa-copy', text: 'Puantaj Ekle' },
            ...
            { id: 'branch_selection', route: 'branch_selection', icon:'fa-copy', text: 'Şube Seçimi' }
        ]
    },
    {
        id: 'current-account',
        ...
        children: [
            { id: 'current_card_list', route: '/current-account/list', icon:'fa-copy', text: 'Cari Kart Listesi' },
            { id: 'add_current_card', route: '/current-account/add', icon:'fa-copy', text: 'Cari Kart Ekle' },
            { id: 'add_current_account_slip', route: 'add_current_account_slip', icon:'fa-copy', text: 'Cari Hesap Fişi Ekle' },
            { id: 'payment_plans', route: 'payment_plans', icon:'fa-copy', text: 'Ödeme Planları' },
    ...
    },
    {
        ...
        children: [
            { id: 'material_stock_list', route: 'material_stock_list', icon:'fa-copy', text: 'Malzeme / Stok Listesi' },
            { id: 'add_stock_card', route: 'add_stock_card', icon:'fa-copy', text: 'Stok Kart Ekle' },
            { id: 'add_material_receipt', route: 'add_material_receipt', icon:'fa-copy', text: 'Malzeme Fişi Ekle' },
    ...
    },
    {
        ...
        route: '/production',
        children: [
            { id: 'product_recipe_list', route: 'product_recipe_list', icon:'fa-copy', text: 'Ürün Reçete Listesi' },
            { id: 'add_product_recipe', route: 'add_product_recipe', icon:'fa-copy', text: 'Ürün Reçetesi Ekle' },
            { id: 'operator_card_list', route: 'operator_card_list', icon:'fa-copy', text: 'Operatör Kart Listesi' },
            ...
            { id: 'production_order_list', route: 'production_order_list', icon:'fa-copy', text: 'Üretim Emri Listesi' },
            { id: 'add_production_order', route: 'add_production_order', icon:'fa-copy', text: 'Üretim Emri Ekle' },
...
function SidebarLeft(props) {
    ...
    return(
        <div className={`flex-none border-r border-gray-normal bg-gray-lightest fixed h-full flex flex-col z-20 transition-all ${isSidebarCollapsed ? 'w-13' : 'w-60'}`}>
            ...
            <div className="overflow-y-auto overflow-x-hidden c-scrollbar-small flex-grow">
                <Link to="/dashboard" className={`p-2 h-13 flex items-center ${location.pathname === '/dashboard' ? 'bg-gray-400' : 'bg-gray-light'}`}>
                    <div className={`relative w-8 h-8 rounded-lg bg-white flex items-center justify-center font-bold text-lg text-gray-darkest`}>
                        Ş1
                        <span className="rounded-full w-5 h-4 bg-blue-700 absolute -top-2 -right-2 text-white flex items-center justify-center text-xs font-normal">99</span>
                    </div>
